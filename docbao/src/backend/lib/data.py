...
# a queue that keep n-last-added item, use for keyword frequent data
...
class Article:
    def __init__(self, article_id, href, topic, date, newspaper, language, sapo, content, feature_image, avatar, post_type, author_id=None, author_fullname='', tags=[]):
        ...
        self._creation_date = pytz.utc.localize(
            datetime.utcnow())  # set utc now time
        self._keywords = []
        self._tokenized = False
        self._language = language
        self._sapo = sapo.strip()
        # content is a list of dict [{'type':'text', 'content':''}, {'type':'image', 'link':'url'}]
    ...
    def get_content_as_html(self, delimiter=' '):
        ...
        for item in self.get_content():
            if item['type'] == 'text':
                ...
                html.append('<img class="aligncenter" src="%s"> <p style="text-align: center;">%s</p> </img>' %
    ...
    def is_quality_content(self, min_word=50, min_long_image=1, min_image=3):
        ...
        #and len(images) >= min_long_image or len(images) >= min_image
        if (len(content.split()) >= min_word ):
...
class ArticleManager:
    ...
    def search_in_database(self, search_string, search_content=True, tag_filter=None, max_number=None):
        ...
        if result:
            sorted_result = sorted(
                result, key=lambda x: x.get_date(), reverse=True)
            if max_number:
    ...
    def investigate_if_link_is_valid_article(self, link, webconfig, home_html_tree, browser, xpath_index, topic_index):
        ...
        if "href" in id_type:
            # to reuse between crawl topic and crawl publish date function
        ...
        if(webconfig.get_topic_from_link()):
            ...
            while a:
                ...
                if "text" in topic_type:
                    ...
                    if (isinstance(result, list)) and len(result) > 0:
                        ...
                        print("Topic found: %s" %
                              trim_topic(topic, max_length))
                        topic_word_list = topic.split()
                    else:
                        print(
                            "Ignore. Extract result none. This link is not an article")
            ...
            # try to crawl topic
            (result, detail_page_html_tree) = self.get_topic_of_an_url(fullurl, webconfig,
                                                                       detail_page_html_tree=detail_page_html_tree, browser=browser, extract_xpath=extract_xpath)
            has_visit = True
        ...
        if(webconfig.get_skip_crawl_publish_date()):
            ...
            if "detail_page" in date_place:
                result = self.get_time_of_an_url(
                    fullurl, webconfig, detail_page_html_tree=detail_page_html_tree, browser=browser, index=topic_index, date_xpath=date_xpath)

                has_visit = True
            else:
                result = self.get_time_of_an_url(
                    fullurl, webconfig, detail_page_html_tree=home_html_tree, browser=browser, index=topic_index, date_xpath=date_xpath)
        if (result is not None):  # found an article
            if result != False:
                ...
                if self.is_not_outdated(newsdate) or webconfig.get_skip_crawl_publish_date():
                    print("Topic publish date (in newspaper timezone): %s" % get_date_string(
                    ...
                    try:
                        if get_detail_content:
                            content_xpath = webconfig.get_content_xpath()[
                                xpath_index]
                            ...
                            try:
                                sapo_xpath = webconfig.get_sapo_xpath()[
                                    xpath_index]
                                sapo = detail_page_html_tree.xpath(sapo_xpath)[
                                    0]
                                # sapo_xpath return an element
                            ...
                            try:
                                content_etree = detail_page_html_tree.xpath(content_xpath)[
                                    0]
                            except:
                                print_exception()
                            ...
                            try:
                                for xpath in ignore_xpaths:
                                    ignore_elements = content_etree.xpath(
                                        xpath)
                            ...
                            # xpath to extract title element
                            image_title_xpaths = webconfig.get_image_title_xpath()

                            # xpath to extract all videobox element
                            video_box_xpaths = webconfig.get_video_box_xpath()
                            # xpath to extract title element
                            video_title_xpaths = webconfig.get_video_title_xpath()

                            # xpath to extract all audiobox element
                            audio_box_xpaths = webconfig.get_audio_box_xpath()
                            # xpath to extract title element
                            audio_title_xpaths = webconfig.get_audio_title_xpath()
                                ...
                                if element in text_elements:
                                    if element not in image_titles and element not in video_titles and element not in audio_titles:
                                        ...
                                        text_content = remove_html(
                                            str(get_tagstring_from_etree(element)), ' ').strip()
                                        content.append(
                                    ...
                                    for image_title_xpath in image_title_xpaths:
                                        try:
                                            ...
                                            image_title = remove_html(
                                                str(get_tagstring_from_etree(element.xpath(image_title_xpath)[0])))
                                            break
                                        except:
                                            print(
                                                "Can't extract image title using %s" % image_title_xpath)

                                    content.append(
                                        {'type': 'image', 'link': image_url, 'content': image_title})
                                    ...
                                    for video_title_xpath in video_title_xpaths:
                                        try:
                                            ...
                                            print(
                                                "Can't extract video title using %s" % video_title_xpath)

                                    content.append(
                                        {'type': 'video', 'link': video_url, 'content': video_title})
                                    ...
                                    for audio_title_xpath in audio_title_xpaths:
                                        try:
                                            ...
                                            print(
                                                "Can't extract audio title using %s" % audio_title_xpath)

                                    content.append(
                                        {'type': 'audio', 'link': audio_url, 'content': audio_title})
                        ...
                        print_exception()
                        print(
                            "Ignore. Can't extract detail content. This might not be an article")
    ...
    def add_url_to_blacklist(self, href):
        ...
        # note: new_blacklist is used for multiprocessing
        self._new_blacklist[href] = self._default_blacklist_count
    ...
    # reward to href when it proves value
    def refresh_url_in_blacklist(self, href):
        self._blacklist[href] = self._default_blacklist_count
        # note: new_blacklist is used for multiprocessing and will be saved, blacklist is readonly in multiprocessing. Without this line will cause program to crawl one item multiple times
        self._new_blacklist[href] = self._blacklist[href]
                ...
                for xpath_index in range(0, len(topics_xpath)):
                    ...
                    for topic_index in range(0, len(topics_list)):
                        ...
                        if "href" in id_type:
                            ...
                            fullurl = remove_html(
                                get_tagstring_from_etree(link))
                            print()
                            print("Crawler pid %s: Processing topic" % my_pid)
                            print(
                                "Note: this website don't use href as id_type. Don't set date_place as detail_page")
                        # epdb.set_trace()
                        if not self.is_blacklisted(fullurl):
                            if not self.is_in_database(fullurl):
                                ...
                                if ('href' in id_type) and (filter.match(fullurl) is None):
                                    print(
                                        "Crawler pid %s: Ignore. This url is from another site" % my_pid)
                                else:
                                    (result, has_visit_page) = self.investigate_if_link_is_valid_article(
                                        link, webconfig, html_tree, browser, xpath_index, topic_index)
                                    if has_visit_page:
                                        count_visit += 1
                                        ...
                                        if result != False:  # valid url
                                            ...
                                            if only_quality_post:
                                                if new_article.is_quality_content():
                                                    self.add_article(
                                                        new_article)
                                                    self.add_url_to_blacklist(
                                                        fullurl)
                                                    count_lay += 1
                                                    print("Crawler pid %s: Crawled articles: %s" % (
                                            ...
                                            if has_visit_page:
                                                ...
                                                print("Crawler pid %s: Waiting %s seconds before continue crawling" % (
                                            ...
                                            # add invalid link to blacklist later to allow all link refer to the same fullurl will be checked
                                            blacklist.append(fullurl)
                                            print(
                                                "Crawler pid %s: Wait finish crawling to add to blacklist" % my_pid)
                                    ...
                                    if count_visit >= maximum_url_to_visit:  # Stop crawling to not get caught by server
                                        print("Crawler pid %s: Stop crawling %s to avoid being caught by server" % (
                                            my_pid, webname))
                                ...
                                print(
                                    "Crawler pid %s: This article has been in database" % my_pid)
                        else:
                            print(
                                "Crawler pid %s: This link is in blacklist database" % my_pid)
    ...
    def get_articles_as_json(self, number=None):
        ...
        for index in range(0, number):
            ...
            if len(article.get_content_as_string()) >=  50:
                count += 1
                update_time = int(
                ...
                json_article_list.append({
                    ...
                    #   'id': article.get_id(),
                      ...
                      str(count): article.get_content_as_string(),
                    #   'update_time': update_time_string,
                    #   'publish_time': article.get_date_string(self._config_manager.get_display_timezone()),
                    #   'newspaper': article.get_newspaper()
                })
    ...
    def export_to_json(self, number=None):
        ...
        with open_utf8_file_to_write(get_independent_os_path(["export", "article_data.json"])) as stream:
            json.dump({'article_list': json_article_list},
